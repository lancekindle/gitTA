...
def trigger(*args):
    ...
    git_dir, _ = os.path.split(git_hooks_dir)  # e.g. '.git'
    path_to_gitta_pyfiles = os.path.join(repo_dir, 'gitTA')  # EVENTUALLY I want to run only code within .git
    ...
    gitTA.gitta.trigger_all_instances(*args[1:], event=event, git_dir=git_dir, repo_dir=repo_dir)
